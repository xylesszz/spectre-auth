  ...
  return (
    <div className="space-y-6">
      ...
      <Card title={`Create ${kind.toLowerCase()} rule`}>
        <form action={async (fd) => { await createRule(fd); }} className="grid grid-cols-2 md:grid-cols-6 gap-3">
          <input type="hidden" name="kind" value={kind} />
          <div>
            <span className={label}>Type</span>
            <select name="type" className={input}>
              {['IP', 'HWID', 'USER', 'LICENSE'].map((t) => <option key={t} value={t}>{t}</option>)}
            </select>
          </div>
          <div className="col-span-2">
            <span className={label}>Value</span>
            <input name="value" required className={input} placeholder="value to block/allow" />
          </div>
          <div>
          ...
          <div>
            <span className={label}>Expires (hours, 0=∞)</span>
            <input name="hours" type="number" min={0} defaultValue={0} className={input} />
          </div>
      ...
      <TableShell colSpan={7} empty={rules.length === 0} head={<><Th>Type</Th><Th>Value</Th><Th>Scope</Th><Th>Reason</Th><Th>Expires</Th><Th>Status</Th><Th></Th></>}>
        {rules.map((r) => (
          <tr key={r.id} className="hover:bg-[#111]">
            ...
            <Td>
              <div className="flex gap-2">
                <ActionForm action={toggleRule.bind(null, r.id)}>
                  <button type="submit" className={btn.yellow}>{r.active ? 'Disable' : 'Enable'}</button>
                </ActionForm>
                <ActionForm action={deleteRule.bind(null, r.id)} confirmText="Delete rule?">
