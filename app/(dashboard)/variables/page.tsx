...
import { PageHeader, Card, Th, Td, TableShell, btn, input } from '@/components/ui';
...
export default async function VariablesPage({ searchParams }: { searchParams: Record<string, string | undefined> }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card title="Set Variable (create or update)">
      ...
      <form className="flex gap-3">
        <select name="app" defaultValue={appFilter ?? ''} className={`${input} w-48`}>
          <option value="">All applications</option>
          {apps.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}
        </select>
        <button className={btn.primary}>Filter</button>
      </form>
      <TableShell colSpan={4} empty={vars.length === 0} head={<><Th>Name</Th><Th>Value</Th><Th>Application</Th><Th></Th></>}>
        {vars.map((v) => (
